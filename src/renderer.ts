...
class Renderer extends EventEmitter<RendererEvents> {
  ...
  constructor(options: RendererOptions) {
    ...
    this.progressCanvas = shadow.querySelector(
      ".progress"
  ...
  private async renderPeaks(
    ...
    const { ctx } = this;
    const barWidth =
      this.options.barWidth != null
        ? this.options.barWidth * devicePixelRatio
        : 1;
    const barGap =
      this.options.barGap != null
        ? this.options.barGap * devicePixelRatio
  ...
  render(audioData: AudioBuffer) {
    ...
    const { devicePixelRatio } = window;
    const parentWidth = this.options.fillParent
      ? this.scrollContainer.clientWidth * devicePixelRatio
      : 0;
    ...
    this.mainCanvas.style.width =
      Math.floor(scrollWidth / devicePixelRatio) + "px";
  ...
  renderProgress(progress: number, autoCenter = false) {
    this.progressCanvas.style.clipPath = `inset(0 ${
      100 - progress * 100
    }% 0 0)`;
