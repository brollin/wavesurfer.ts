...
> {
  ...
  private handleMouseMove = (e: MouseEvent) => {
    ...
    if (this.options.dragSelection && !isNaN(this.dragStart)) {
      const dragEnd =
        e.clientX - this.regionsContainer.getBoundingClientRect().left;
  ...
  private updateRegion(region: Region, start?: number, end?: number) {
    ...
    if (start != null) {
      ...
      region.startTime =
        (start / this.regionsContainer.clientWidth) *
    ...
    if (end != null) {
      ...
      region.element.style.width = `${region.end - region.start}px`;
      region.endTime =
        (end / this.regionsContainer.clientWidth) *
        this.wavesurfer.getDuration();
    }
  ...
  public setRegionColor(region: Region, color: string) {
    region.element.style[
      region.startTime === region.endTime ? "borderColor" : "backgroundColor"
    ] = color;
  }
}
